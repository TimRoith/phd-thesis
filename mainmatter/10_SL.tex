...
%
%
\subsection{Stochastic and Momentum Variants}
%
%
...
%
%
where $\func_l$ is choosen according to the layer type. In the easiest case of a fully connected layer $\Psi^l$, we can choose
%
\begin{align*}
\func^l(W^l) := \norm{W^l}_1.
\end{align*}
...
\end{align*}
%
The outer sum acts as a $L^1$ regularizer on the instances $\norm{K_{i,j}}$. Sparsity in this sense, then amounts to having indices $(i,j)$ for which $\norm{K_{i,j}}=0 \Leftrightarrow K_{i,j}=0$, i.e., we prune away whole convolutional filters.
%
We can also employ group sparsity on fully connected layers, by considering row sparsity
%
\begin{align*}
\func^l(W^l)
\end{align*}
\paragraph{Comments on the Numerical Results}
