...
\end{align*}
%
where we assume an ordering $\domain_n=\{x_1,\ldots, x_n\}$. In order to have a computationally feasible problem this matrix should have very few non-zero elements. Assuming that the kernel $\eta$ has compact support we observe that the sparsity is directly influenced by the graph scale $\gscale$.
%
\begin{figure}
\centering
\includegraphics[width=.5\textwidth]{code/SSL/Times.pdf}
\caption{Times this that, Gecko hat}
\end{figure}
