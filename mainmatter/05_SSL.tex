...
%
%
Let $\vec u_1, \vec u_2$ be two  solutions of \cref{prob:vargraph}, i.e. $\GE^{w_n}_p (\vec u_1)= \GE^{w_n}_p (\vec u_2)$. Assume that there exists $(\bar x,\bar y)\in E$ such that $\nabla^{w_n} \vec u_1(\bar x,\bar y)\neq \nabla^{w_n}\vec u_2(\bar x,\bar y)$, then we have that
%
\begin{align*}
...
%
\begin{gather*}
\abs{\vec u(x)}^p \leq \sum_{i=1}^{m^x-1}\abs{\vec u(\gamma^x_{i}) - \vec u(\gamma^x_{i+1})}^p + \abs{\vec g(o^x)}^p\leq 
c \GE^{w_n}_p(\vec u) + \abs{\vec g(o^x)}^p
\end{gather*}
%
where $c=\left(\min_{(x,y)\in E}w_n(x,y)\right)^{-1}$. Summing over all $x\in\domain_n$ yields

\begin{gather*}
\norm{\vec u}^p_p = \sum_{x\in\domain_n} \abs{\vec u(x)}^p \leq 
n c\ \GE^{w_n}_p(\vec u) + \sum_{x\in\domain_n} \abs{\vec g(o^x)}^p\\
%
\leq  C \bigg(\GE^{w_n}_p(\vec u) + \norm{\vec g}^p_p\bigg)
\end{gather*}
%
where $C=\max\{nc, 1\}$. Let now $\vec u_j$ denote a sequence such that
%
\begin{align*}
...
\end{align*}
%
for functions of class $C^2$. For any vector $v\in\R^n$ and $h\in\R^+$ we can employ the Taylor approximation to see that
%
\begin{align*}
