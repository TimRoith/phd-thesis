...
%
\begin{align*}
(\nabla^{w_n} \vec u)_{(x,y)} := w_n(x,y) \abs{\vec u(y) - \vec u(x)}
\end{align*}
%
the \emph{graph gradient} on $E$. We have that $\GE^{w_n}_p (\vec u) = \norm{\nabla^{w_n}\vec u}$. Let $\vec u_1, \vec u_2$ be two  solutions of \cref{prob:vargraph}, i.e. $\GE^{w_n}_p (\vec u_1)= \GE^{w_n}_p (\vec u_2)$. Assume that there exists $(\bar x,\bar y)\in E$ such that $\nabla^{w_n} \vec u_1(\bar x,\bar y)\neq \nabla^{w_n}\vec u_2(\bar x,\bar y)$, then we have that
%
\begin{align*}
...
\vec u_1(x) - \vec u_1(y) = \vec u_2(x) - \vec u_2(y) \qquad\text{for all}\qquad (x,y)\in E.
\end{align}
%
...
%
\begin{align*}
\vec u_1(\gamma_{i+1}) - \vec u_1(\gamma_i) = \vec u_2(\gamma_{i+1}) - \vec u_2(\gamma_i) \qquad\text{for all}\qquad i=1,\ldots,m-1.
\end{align*}
%
and therefore
%
\begin{align*}
\vec u_1(x) - \vec u_1(o) &= \left[\sum_{i=1}^{m-1} \vec u_1(\gamma_{i+1}) - \vec u_1(\gamma_i)\right] + \vec u_1(o)
= \left[\sum_{i=1}^{m-1} \vec u_2(\gamma_{i+1}) - \vec u_2(\gamma_i)\right]\\&=
\vec u_2(x) - \vec u_2(o).
\end{align*}
%
Employing the boundary conditions $\vec u_1(o)=\vec u_2(o)$, it follows that $\vec u_1(x) = \vec u_2(x)$. Since $x\in\domain_n$ was arbitrary, we get $\vec u_1 = \vec u_2$.\par
%
\noindent%
To show existence, we do not need to assume that the graph is connected to the boundary. In fact one easily observes that being constant, on every connected component that is not connected to $\conset$ is optimal. W.l.o.g. we neglect connected components not connected to the boundary and see that for $\vec u:\domain_n\to\R$ and any $x\in\domain_n$ we have that
%
\begin{align*}
\abs{\vec u(x)}^p \leq \sum_{i=1}^{m-1}\abs{\vec u(\gamma_{i+1}) - \vec u(\gamma_i)}^p + \abs{\vec g(o)}^p 
\end{align*}
%
where we employed a path to the boundary as in the previous example. Therefore, we obtain that $\norm{\vec u}^p_p \leq C (\GE^{w_n}_p(\vec u) + \norm{\vec g}^p_p)$. Let now $\vec u_j$ denote a sequence such that $\GE^{w_n}_p(\vec u_j) \leq I + 1/j$, where
%
\begin{align*}
...
%
\begin{align*}
\norm{\vec u_j}^p_p \leq C(I + 1/j + \norm{\vec g}^p_p) \leq \tilde C
\end{align*}
%
i.e. $\vec u_j$ is a bounded sequence of vectors in the finite dimensional space $\R^n$, and therefore there exists a subsequence---which we do not relabel---that converges to $\vec u$. Since the functional $\GE^{w_n}_p:\R^n\to\R$ is continuous we obtain that
%
\begin{align*}
\GE^{w_n}_p(\vec u) = \lim_{j\to\infty} \GE^{w_n}_p(\vec u_j) = I
\end{align*}
%
...
%
 ...
 While there are many examples where this simple concept does in fact yield good solutions \cite{zhu2003semi, zhou2005regularization}, it has been observed that solutions tend to degenerate for large amounts of data, whenever $d\geq 2$ \cite{nadler2009statistical, alamgir2011phase, el2016asymptotic}. We illustrate this phenomenon in \cref{ex:pLapBad}.
%
\begin{figure}
...
As observed in \cite{nadler2009statistical, alamgir2011phase, el2016asymptotic} the question of consistency is now connected to relation between $p$ and $d$, where divides into the three regimes $p<d$, $p=d$ and probably the most relevant case $p>d$.
